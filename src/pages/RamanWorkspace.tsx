...
import { getStepParameterDefinitions, RAMAN_PARAMETER_DEFINITIONS } from '../data/parameterDefinitions';
...
export default function RamanWorkspace() {
  ...
  const [parameters, setParameters] = useState<Record<string, Record<string, any>>>(() => {
    ...
    Object.entries(RAMAN_PARAMETER_DEFINITIONS).forEach(([stepId, defs]) => {
      defaults[stepId] = {};
  ...
  const handleResetParameters = () => {
    if (activeStep) {
      const defs = getStepParameterDefinitions('raman', activeStep);
  ...
  return (
    <DashboardLayout>
      ...
      <div className="flex-1 h-full flex flex-col overflow-hidden bg-background">
        ...
        <aside className="w-[280px] border-r border-border bg-surface flex flex-col shrink-0 overflow-hidden">
          ...
          <div className="border-b border-border px-3 py-2">
            <h1 className="text-xs font-bold text-text-main leading-tight">Raman Workspace</h1>
            <p className="text-[9px] text-text-muted leading-tight">Vibrational mode evidence review</p>
      ...
      <ParameterDrawer
        ...
        parameterDefinitions={activeStep ? getStepParameterDefinitions('raman', activeStep) : []}
